...
#define INPUT_SIZE 10           // Changed to 10 for numbers 0 to 9
...
void writeLogFile(Individual population[POPULATION_SIZE], int generation, double elapsedSeconds) {
    FILE *file = fopen("logNumNet.txt", "w");
    if (file == NULL) {
    ...
    fprintf(file, "Genes: ");
    for (int i = 0; i < INPUT_SIZE; ++i) {
        fprintf(file, "%c", population[0].genes[i] == '1' ? '0' + i : '-');
    }
...
void evolvePopulation(Individual population[POPULATION_SIZE], int *generation, clock_t *start, int targetFitness, int *learningFailed) {
    ...
    for (int i = 0; i < INPUT_SIZE; ++i) {
        printf("%c", population[0].genes[i] == '1' ? '0' + i : '-');
    }
    ...
    if (checkCorrectOrder(population[0])) {
        printf("\n\033[1;32mSuccessfully Learned Numbers at generation: %d\n", *generation - 1);
...
int main() {
    system("title Number Learning Neural Network");
